"""
ITC Q-Commerce Price Tracker  v4.0
- Waits explicitly for prices to load before reading page
- No CSS card selector dependency — reads full page text
- No discount % column
- 4 categories: Atta, Nimyle, Yippee, Shower Gel
- 3 pincodes, 3 platforms
"""
...
SKUS = [
    ("AA_AT_5K",  "Aashirvaad","Aashirvaad Atta 5kg",            "Foods","Atta",         280,True, "aashirvaad atta 5kg"),
    ("AA_AT_10K", "Aashirvaad","Aashirvaad Atta 10kg",           "Foods","Atta",         520,True, "aashirvaad atta 10kg"),
    ("GN_AT_5K",  "Ganesh",    "Ganesh Atta 5kg",                "Foods","Atta",         250,False,"ganesh atta 5kg"),
    ("GN_AT_10K", "Ganesh",    "Ganesh Atta 10kg",               "Foods","Atta",         480,False,"ganesh atta 10kg"),
    ("NM_FC_1L",  "Nimyle",    "Nimyle Floor Cleaner 1L",        "Personal Care","Floor Cleaner",120,True, "nimyle floor cleaner 1l"),
    ("NM_FC_2L",  "Nimyle",    "Nimyle Floor Cleaner 2L",        "Personal Care","Floor Cleaner",210,True, "nimyle floor cleaner 2l"),
    ("LZ_FC_1L",  "Lizol",     "Lizol Floor Cleaner 1L",         "Personal Care","Floor Cleaner",185,False,"lizol floor cleaner 1l"),
    ("YP_MM_4P",  "Yippee",    "Yippee Magic Masala 4 Pack",     "Foods","Noodles",       60,True, "yippee magic masala noodles"),
    ("YP_CM_4P",  "Yippee",    "Yippee Classic Masala 4 Pack",   "Foods","Noodles",       60,True, "yippee classic masala noodles"),
    ("NS_MG_4P",  "Maggi",     "Maggi Masala Noodles 4 Pack",    "Foods","Noodles",       60,False,"maggi masala noodles 4 pack"),
    ("FM_SG_BC",  "Fiama",     "Fiama Shower Gel Blackcurrant 250ml","Personal Care","Shower Gel",299,True,"fiama shower gel blackcurrant"),
    ("FM_SG_PH",  "Fiama",     "Fiama Shower Gel Peach 250ml",   "Personal Care","Shower Gel",299,True,"fiama shower gel peach"),
    ("DV_BW_250", "Dove",      "Dove Body Wash 250ml",           "Personal Care","Shower Gel",299,False,"dove body wash 250ml"),
]
...
# ── MATCH SKU IN TEXT BLOCK ───────────────────────────────────
def best_price_from_text(query, full_text, mrp):
    """
    Split page into chunks around price signs.
    Find chunk that best matches query and return lowest price.
    """
    low = full_text.lower()
    # Core keywords from query (ignore sizes)
    core = re.sub(r'\b(\d+g|\d+ml|\d+kg|\d+l|\d+ pack|pack of \d+|\d+s?)\b','', query.lower())
    words = [w for w in re.split(r'\W+', core) if len(w) > 2]
    if not words:
        return None

    # Split text into ~200 char windows around each ₹ sign
    best_price = None
    for m in re.finditer(r'₹\s*(\d+)', full_text):
        val = int(m.group(1))
        if val < 5 or val > mrp * 3:
            continue
        # Window of text around this price
        start = max(0, m.start() - 150)
        end   = min(len(full_text), m.end() + 150)
        window = full_text[start:end].lower()
        # Check how many keywords match in this window
        matches = sum(1 for w in words if w in window)
...
async def set_location(page, pincode, platform):
    try:
        ...
        # Try clicking location bar
        for sel in ["[data-testid='location-bar']","[class*='LocationBar']",
                    "[class*='location-bar']","text=Detect Location",
                    "text=Enter Location","text=Set Location"]:
            try:
        ...
        for sel in ["input[placeholder*='pincode' i]","input[placeholder*='area' i]",
                    "input[placeholder*='enter' i]","input[placeholder*='search' i]",
                    "input[type='search']","input[type='text']"]:
            try:
                inp = page.locator(sel).first
                if not await inp.is_visible(timeout=2000): continue
                await inp.click()
                await inp.fill("")
        ...
        # Fallback: check if page has any prices visible
...
# ── SEARCH & EXTRACT PRICE ────────────────────────────────────
async def get_price(page, sku, platform):
    sid, brand, name, cat, sub, mrp, is_itc, query = sku
    try:
        url = SEARCH_URLS[platform].format(q=query.replace(" ", "+"))
        await page.goto(url, timeout=35000, wait_until="domcontentloaded")

        # ── KEY FIX: wait until ₹ symbol appears on page ──────
        try:
            await page.wait_for_selector("text=₹", timeout=8000)
        except:
            # Also try Rs.
            try:
                await page.wait_for_selector("text=Rs", timeout=3000)
            except:
                return None

        # Extra buffer for all cards to render
        await page.wait_for_timeout(2000)
        ...
        price = best_price_from_text(query, full_text, mrp)
        return price
...
h1{font-size:1.1rem;font-weight:700}.sub{font-size:.7rem;color:var(--mu);font-family:'DM Mono',monospace;margin-top:2px}
.rt{font-size:.68rem;color:var(--mu);font-family:'DM Mono',monospace;text-align:right}.rt b{color:var(--gold)}
...
.filters{padding:14px 28px;border-bottom:1px solid var(--bd);display:flex;flex-wrap:wrap;gap:14px;background:var(--sf);align-items:flex-start}
...
#search{background:var(--bg);border:1px solid var(--bd);color:var(--tx);padding:5px 11px;border-radius:8px;font-family:'Syne',sans-serif;font-size:.78rem;width:160px;outline:none}
...
td{padding:7px 13px;white-space:nowrap}td.nm{white-space:normal;max-width:240px;font-weight:500}
...
.pr{font-family:'DM Mono',monospace;font-weight:700;color:var(--gn);font-size:.85rem}
...
.na{color:#2d3f5a;font-family:'DM Mono',monospace}
...
<header>
  ...
  <div class="rt">Updated<br><b id="rdt"></b></div>
</header>
<div class="stats">
  <div class="stat"><div class="sv" id="ss">—</div><div class="sl">SKUs</div></div>
  <div class="stat"><div class="sv" id="sf">—</div><div class="sl">Prices Found</div></div>
...
<div class="tabs">
  ...
  <div class="tab" data-view="v2">📊 Platform Comparison</div>
...
<div class="tw">
  ...
  <div class="view" id="v3">
    <table><thead><tr>
      ...
      <th>Comp Price</th><th>Verdict</th>
...
<script>
const R = """ + data_str + """;
const S={platform:new Set(['ALL']),category:new Set(['ALL']),type:new Set(['ALL']),brand:new Set(['ALL']),search:''};
let sc=null,sd=1;
...
function fr(){
  return R.filter(r=>{
    ...
    if(!S.category.has('ALL')&&!S.category.has(r.subcategory))return false;
...
function pvt(rows){
  const m={};
  rows.forEach(r=>{if(!r.sell_price)return;
    const k=r.sku_id;
  ...
  return Object.values(m).map(r=>{
    ...
    r.min_price=ps.length?Math.min(...ps):null;return r;
  });
}
const PAIRS=[
  ['Atta','Aashirvaad','Ganesh'],
  ['Floor Cleaner','Nimyle','Lizol'],
...
function cmps(rows){
  return PAIRS.map(([sub,ib,cb])=>{
    ...
    return{sub,ib,is:i.sku_name,ip:i.sell_price,ipl:i.platform,cb,cs:c.sku_name,cp:c.sell_price,d};
  }).filter(Boolean);
}
...
function render(){
  ...
  document.getElementById('tb1').innerHTML=rows.map(r=>`<tr>
    <td class="${r.is_itc?'bi':'bc'}">${r.brand}</td>
  ...
  document.getElementById('tb2').innerHTML=pvt(rows).map(r=>`<tr>
    <td class="${r.is_itc?'bi':'bc'}">${r.brand}</td>
    <td class="nm">${r.sku_name}</td>
    ...
    <td>${r.min_price?`<span class="pr" style="font-size:.9rem">₹${r.min_price}</span>`:'<span class="na">—</span>'}</td>
  </tr>`).join('');
  document.getElementById('tb3').innerHTML=cmps(rows).map(r=>{
    ...
    return`<tr>
      ...
      <td>${r.d!==null?(r.d<=0?'✅ ITC cheaper by '+Math.abs(r.d)+'%':'⚠️ ITC costlier by '+r.d+'%'):'—'}</td>
    </tr>`;
  }).join('');
...
bp('fp','platform',uniq('platform'));bp('fc','subcategory',uniq('subcategory'));bp('fb','brand',uniq('brand'));
    ...
    async with async_playwright() as pw:
        ...
        for (pincode, area, tier) in PINCODES:
            ...
            for platform in PLATFORMS:
                ...
                for sku in SKUS:
                    sp = await get_price(page, sku, platform)
                    key = (sku[0], platform)
                    if sp:
                        ex = best.get(key)
                        if not ex or sp < ex["sell_price"]:
                            best[key] = {"sell_price":sp,"pincode_min":pincode}
    ...
    for sku in SKUS:
        ...
        for platform in PLATFORMS:
            ...
            results.append({
                ...
                "sell_price":sp,"pincode_min":b.get("pincode_min",""),
                "checked_at":run_dt,
            })
